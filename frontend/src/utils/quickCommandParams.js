 ...
 * @returns {Array<{num: number, label: string}>} 参数列表，按 num 升序
 */
export function extractQuickCommandParams(command) {
 ...
 * @param {string} command - 命令字符串
 * @param {Record<number, string>} values - 参数值，键为参数编号
 * @returns {string} 替换后的命令
 */
export function fillQuickCommandParams(command, values) {
